...
info:
  ...
  x-origin:
    - converter:
        ...
        version: 2.6.2
      format: openapi
      url: 'https://raw.githubusercontent.com/adyen/adyen-openapi/master/specs/3.0/CheckoutService-V30.json'
      version: '3.0'
  x-preferred: false
      ...
      properties:
        ...
        ownerName:
          description: |-
            ...
            * Ï‡12 is converted to ch12.
            * Ã¼A is converted to euA.
            * Peter MÃ¸ller is converted to Peter Mller, because banks don't accept 'Ã¸'.
      ...
      properties:
        cvc:
          description: |-
            ...
            * CVV2/CVC2 â€“ length: 3 digits
            * CID â€“ length: 4 digits
      ...
      properties:
        ...
        recurringProcessingModel:
          description: |-
            ...
            Allowed values:
            * `Subscription` â€“ A transaction for a fixed or variable amount, which follows a fixed schedule.
            * `CardOnFile` â€“ Card details are stored to enable one-click or omnichannel journeys, or simply to streamline the checkout process. Any subscription not following a fixed schedule is also considered a card-on-file transaction.
          enum:
            - CardOnFile
            - Subscription
          type: string
      ...
      properties:
        contract:
          description: |-
            ...
            Possible values:
            * `ONECLICK` â€“ The shopper opts to store their card details for future use. The shopper is present for the subsequent transaction, for cards the security code (CVC/CVV) is required.
            * `RECURRING` â€“ Payment details are stored for future use. For cards, the security code (CVC/CVV) is not required for subsequent payments. This is used for shopper not present transactions.
            * `ONECLICK,RECURRING` â€“ Payment details are stored for future use. This allows the use of the stored payment details regardless of whether the shopper is on your site or not.
          enum:
            - ONECLICK
            - RECURRING
            - PAYOUT
