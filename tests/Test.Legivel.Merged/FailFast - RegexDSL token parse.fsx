...
type CharacterMatch =
    |   Indecisive      = 0 //  no decicsion made yet
    |   NoMatch         = 1 //  decided no match
    |   Match           = 2 //  decided match
    |   MatchContinue   = 3 //  decided match, but may receive more (with repetitions)
    |   MatchReduce     = 4 //  decided match, but previous did not match and must be reduced

type ThompsonState = {
    State           :   CharacterMatch
    CharCount       :   int
    IterationCount  :   int
    InitFuncs       :   (ThompsonState * ThompsonFunc) list
    FunctionState   :   (ThompsonState * ThompsonFunc) list  // function specific state
}
with    
    static member Create() = { State = CharacterMatch.Indecisive; CharCount = 0; InitFuncs = []; FunctionState = []; IterationCount=0}
    static member Create fs = { State = CharacterMatch.Indecisive; CharCount = 0; InitFuncs = fs; FunctionState = fs;IterationCount=0}
    member this.SetState s = { this with State =  s; }
    member this.SetState fs = { this with FunctionState = fs }
    member this.SetState (s, fs) = { this with State =  s; FunctionState = fs }
    member this.IncreasCharCount() = { this with CharCount = this.CharCount + 1}
    member this.Iterate() = { this with IterationCount = this.IterationCount + 1 }
    member this.ReinitFuncs() = { this with FunctionState = this.InitFuncs }
and ThompsonFunc = (ThompsonState*TokenData) -> ThompsonState

let increaseCharCount (ts:ThompsonState) = ts.IncreasCharCount()
let iterate (ts:ThompsonState)  = ts.Iterate()
let reinit (ts:ThompsonState) = ts.ReinitFuncs()
let callFirst (ts:ThompsonState, td:TokenData) = 
    let (st,fn) = ts.FunctionState |> List.head
    fn (st, td)


let boolToMatch (ts:ThompsonState) = 
    function
    |   true -> ts.SetState(CharacterMatch.Match)
    |   false -> ts.SetState(CharacterMatch.NoMatch)
    ...
    fun (ts:ThompsonState, td:TokenData) ->
        let operation =
            ...
            |>  List.map(fun (ts,tf) ->
                ...
                |   CharacterMatch.Indecisive   -> tf(ts, td), tf
                |   CharacterMatch.Match        -> failwith "we should never be here, parsing must stop before this situation"
                |   CharacterMatch.MatchReduce
                |   CharacterMatch.NoMatch      -> ts, tf // return what we have
                |   _ -> failwith "Illegal state"
            )

        let extractedStatus =
            operation 
            |>  List.fold(fun (m, mr, nm, l) (ts, _) -> 
                match ts.State with
                |   CharacterMatch.Match       -> (m+1, mr, nm, l+1)
                |   CharacterMatch.MatchReduce -> (m, mr+1, nm, l+1)
                |   CharacterMatch.NoMatch     -> (m, mr, nm+1, l+1)
                |   _                          -> (m, mr, nm, l+1)
            ) (0, 0, 0, 0)

        match extractedStatus with
        |   (m, mr, nm, l) when mr > 0 -> ts.SetState(CharacterMatch.MatchReduce, operation)
        |   (m, mr, nm, l) when m  > 0 -> ts.SetState(CharacterMatch.Match, operation)
        |   (m, mr, nm, l) when nm = l -> ts.SetState (CharacterMatch.NoMatch, operation)
        |   _                          -> ts.SetState (CharacterMatch.Indecisive, operation)

let concatParser() = 
    fun (ts:ThompsonState, td:TokenData) ->
        let rec concat ts lst =
            match lst with
            |   [(ts,tf)]  -> 
                let r : ThompsonState = tf(ts,td)
                r.SetState []
            |   (ts,tf) :: tail -> 
                let r = tf(ts,td)
                r.State
                |>  function
                |   CharacterMatch.MatchContinue
                |   CharacterMatch.Indecisive   -> r.SetState ((r,tf) :: tail)
                |   CharacterMatch.NoMatch      -> r.SetState []                               // empty concat list, no continu
                |   CharacterMatch.Match        -> r.SetState (CharacterMatch.Indecisive, tail)// advance in the concat list, but no full match yet
                |   CharacterMatch.MatchReduce  -> concat r tail
                |   _ -> failwith "Illegal state"
        concat ts ts.FunctionState
...
let optionalParser() =
    fun (ts:ThompsonState, td:TokenData) ->
        ...
        |>  function
        |   CharacterMatch.MatchContinue
        |   CharacterMatch.Indecisive
        |   CharacterMatch.Match
        |   CharacterMatch.MatchReduce  -> r
        |   CharacterMatch.NoMatch      -> r.SetState CharacterMatch.Match
        |   _ -> failwith "Illegal state"
...
let CreatePushParser (rgx:RGXType) =
    let rec getParser rgx : (ThompsonState*ThompsonFunc) =
        match rgx with
        |   Plain    pl -> 
            if pl.``fixed``.Length > 1 then
                pl.OptimizeOnce()
                ...
                ThompsonState.Create(), plainParser pl
        |   OneInSet ois -> ThompsonState.Create(), oneInSetParser ois
        |   Or       l ->
            let orList = l |> List.rev |> List.map(fun i -> getParser i)
            ThompsonState.Create orList, orParser()
        |   Concat   l ->
            let concatList = l |> List.rev |> List.map(fun i -> getParser i)
...
let ``Parse Plain Character - sunny day``() =
    let (st,fn) = RGP("A", [Token.``c-printable``]) |> CreatePushParser
    let yaml = "A"
...
let ``Parse Plain String - sunny day``() =
    let (st,fn) = RGP("ABC", [Token.``c-printable``]) |> CreatePushParser
    
    let rec processor (streamReader:RollingStream<TokenData>) state =
        if state.State <> CharacterMatch.Indecisive then
            state
        else
            let tk = streamReader.Get()
            if tk.Token = Token.EOF then state
            else
                fn (state, tk) |> processor streamReader


    let yaml = "ABC"
    let streamReader = RollingStream<_>.Create (tokenProcessor yaml) EndOfStream
    let r = processor streamReader st
    Assert.AreEqual(CharacterMatch.Match, r.State)
    Assert.AreEqual(Token.EOF, streamReader.Get().Token)
    ...
    let streamReader = RollingStream<_>.Create (tokenProcessor yaml) EndOfStream
    let r = processor streamReader st
    Assert.AreEqual(CharacterMatch.NoMatch, r.State)
    Assert.AreEqual(Token.EOF, streamReader.Get().Token)
