...
module ParseState = 
    ...
    let ``Match and Advance`` (patt:NFAMachine) (postAdvanceFunc:ParseState -> FallibleOption<_>*ParseMessage) (ps:ParseState) =
    ...
    let inline ``Match and Parse`` (patt:NFAMachine) (parseFunc: string -> ParseState -> FallibleOption<_> * ParseMessage) (ps:ParseState) =
        match (HasMatches(ps.Input.Data, patt)) with
    ...
    let PostProcessErrors fr =
        ...
        |> FallibleOption.map(fun (node, ps:ParseState) ->
            let rg = 
                RGP("---", [Token.``t-hyphen``]) ||| RGP("...", [Token.``t-dot``])
                |>  rgxToNFA
...
with
    static member Create n i s c = { Name = n; Indent = i; SubIndent = s; Context = c}


[<NoComparison>]
type NFACache = private {
    Cached : Dictionary<NFACacheKey, NFAMachine>
}
with
    static member Create() = { Cached = new Dictionary<NFACacheKey, NFAMachine>() }
    member this.Clear() = this.Cached.Clear()
    member this.GetOrConvert n (ps:ParseState) (rg: RGXType)  =
        let k = NFACacheKey.Create n ps.n ps.m ps.c
        if this.Cached.ContainsKey(k) then
           this.Cached.[k]
        else
            let conv = rg |> rgxToNFA
            this.Cached.Add(k, conv)
            conv

let nfaCache = NFACache.Create()
...
[<DebuggerStepThrough>]
let (|Regex3|_|) (nfa:NFAMachine) (ps:ParseState) =
    AssesInput (ps.Input.Data) nfa
    |>  function
        |   rs when rs.IsMatch ->
            let fullMatch = rs.FullMatch |> charListToString
            let ps = ps.Advance()
            let groups = rs.Groups |> List.map(charListToString)
...
[<DebuggerStepThrough>]
let (|Regex4|_|) (nfa:NFAMachine, condition:(RollingStream<TokenData> * TokenData-> bool)) (ps:ParseState) =
    //AssesInputPostParseCondition condition (ps.Input.Data) pattern
    //AssesInput (ps.Input.Data) nfa
    //|>  TokenDataToString
    //|>  Option.bind(fun inps -> 
    //    let m = Regex.Match(inps, (stringify nfa), RegexOptions.Multiline)
    //    if m.Success then 
...
//#endif
    ...
    member this.``auto detect indent in line`` ps =
        let icp = 
            GRP(ZOM(RGP (this.``s-space``, [Token.``t-space``]))) + OOM(this.``ns-char``)
            |>  rgxToNFA
        let p = ps.Input.Position
        ...
        match tkl with
        |   tl  when tl.IsMatch -> 
            let th = tl.FullMatch |> List.takeWhile(fun e -> e = ' ')
            th.Length - ps.n
        |   _ -> -1
            ...
            |   Regex(RGSF(this.``ns-global-tag-prefix``)) _ -> 
                ...
                |>  function
                    ...
                    |   None    -> 
                        ...
                        |   Scalar  -> FallibleOption.Value(Unrecognized (this.GlobalTagSchema.UnresolvedResolution (node.Kind) (tsh.MappedTagBase+sub)) |> node.SetTag, ps |> ParseState.IncUnrecognizedScalar), ps.Messages
    ...
    member this.``NFA s-separate`` ps = this.``s-separate`` ps |> nfaCache.GetOrConvert "s-separate" ps
    member this.``NFA optional s-separate`` ps = nfaCache.GetOrConvert "optional s-separate" ps (OPT(this.``s-separate`` ps))
    ...
    member this.``l-directive`` (ps:ParseState) : FallibleOption<ParseState>*ParseMessage = 
        let ParseDirective prs : FallibleOption<Directive*ParseState>*ParseMessage =
            //let ``ns-yaml-directive`` = this.``ns-yaml-directive`` + this.``s-l-comments``  |> rgxToNFA
            //let ``ns-tag-directive`` = this.``ns-tag-directive``  + this.``s-l-comments``   |> rgxToNFA
            //let ``ns-reserved-directive`` = GRP(this.``ns-reserved-directive``) + this.``s-l-comments`` |> rgxToNFA

            match prs with
            |   Regex3(HardValues.``ns-yaml-directive with comments``)  (mt, ps) -> 
                //let ps = 
            ...
            |   Regex3(HardValues.``ns-tag-directive with comments``)   (mt, ps)   -> 
                let tg = mt.ge2 |> fst
            ...
            |   Regex3(HardValues.``ns-reserved-directive with comments``) (mt, ps) -> 
        ...
        ps 
        |> ParseState.``Match and Advance`` HardValues.``NFA Percent`` (ParseDirective)
        |> FallibleOption.bind(fun (t,prs) ->
            prs 
            |> ParseState.``Match and Advance`` HardValues.``NFA s-l-comments`` 
    ...
    member this.``c-ns-properties`` ps : FallibleOption<ParseState * (TagKind*DocumentLocation) * (string*DocumentLocation)>*ParseMessage =
        ...
        let anchor pst  =
            pst |> ParseState.``Match and Advance`` HardValues.``NFA Ampersand`` (fun psr ->
                match psr with
                |   Regex3(HardValues.``NFA ns-anchor-name``) (mt,prs) -> FallibleOption.Value(prs, mt.FullMatch), prs.Messages
                |   Regex3(HardValues.``NFA illegal ns-anchor-name``) (mt,prs) -> psr.AddErrorMessage <| MessageAtLine.CreateContinue (ps.Location) MessageCode.ErrAnchorSyntax (lazy sprintf "Anchor has incorrect format: &%s" mt.FullMatch)
        ...
        let tag pst =
            match pst with
            |   Regex3(HardValues.``NFA Illegal Verbatim No Local Tag``) _ -> pst.AddErrorMessage <| MessageAtLine.CreateContinue (ps.Location) MessageCode.ErrVerbatimTagNoLocal (lazy "Verbatim tags aren't resolved, so ! is invalid.")
            |   Regex3(HardValues.``NFA Verbatim``) (mt, prs) -> 
                let tag = mt.ge1
                if System.Uri.IsWellFormedUriString(tag, UriKind.Absolute) || IsMatchStr(tag, HardValues.``NFA tagged uri``) then
            ...
            |   Regex3(HardValues.``NFA Illegal Verbatim``) _ -> pst.AddErrorMessage <| MessageAtLine.CreateContinue (ps.Location) MessageCode.ErrVerbatimTag (lazy "Verbatim tag starting with '!<' is missing a closing '>'")
            |   Regex3(HardValues.``NFA Shorthand Named``) (mt, prs) -> FallibleOption.Value(prs, ShortHandNamed mt.ge2),prs.Messages
            |   Regex3(HardValues.``NFA Illegal Shorthand Named``) (mt,_) -> pst.AddErrorMessage <| MessageAtLine.CreateContinue (ps.Location) MessageCode.ErrShorthandNamed (lazy sprintf "The %s handle has no suffix." mt.FullMatch)
            |   Regex3(HardValues.``NFA Shorthand Secondary``) (mt, prs) -> FallibleOption.Value(prs, ShortHandSecondary mt.ge1),prs.Messages
            |   Regex3(HardValues.``NFA Illegal Shorthand Secondary``) _ -> pst.AddErrorMessage <| MessageAtLine.CreateContinue (ps.Location) MessageCode.ErrShorthandSecondary (lazy "The !! handle has no suffix.")
            |   Regex3(HardValues.``NFA Shorthand Primary``) (mt, prs) -> FallibleOption.Value(prs, ShortHandPrimary mt.ge1),prs.Messages
            |   Regex3(HardValues.``NFA c-non-specific-tag``) (_, prs) -> FallibleOption.Value(prs, NonSpecificQT),prs.Messages
            |   _ -> FallibleOption.NoResult(), ps.Messages
            |> FallibleOption.map(fun (p,t) -> p,(t,pst.Location))

        let matchTagAnchor pst =
            let afterTag ps tg =
                let ``NFA s-separate`` = this.``NFA optional s-separate`` ps
                ps
                |> ParseState.``Match and Advance`` ``NFA s-separate`` anchor
        ...
        let matchAnchorTag pst =
            let afterAnchor ps anch =
                let ``NFA s-separate`` = this.``NFA optional s-separate`` ps
                ps
                |> ParseState.``Match and Advance`` ``NFA s-separate`` tag
    ...
    member this.``c-ns-alias-node`` ps : ParseFuncResult<_> =
        ...
        ps |> ParseState.``Match and Advance`` HardValues.``NFA Asterisk`` (fun prs ->
            prs |> ParseState.``Match and Parse`` HardValues.``NFA ns-anchor-name`` (fun mt prs2 ->
    ...
    member this.``c-double-quoted`` ps : ParseFuncResult<_> = 
        ...
        let processFunc ps =
            ...
            let patt = 
                this.``c-double-quote`` + GRP(this.``nb-double-text`` ps) + this.``c-double-quote``
                |> nfaCache.GetOrConvert "NFA nb-double-text" ps

            let ``illegal-chars`` = 
                this.``c-double-quote`` + OOM((this.``nb-json`` - this.``c-double-quote``) ||| this.``s-double-break`` ps) + this.``c-double-quote``
                |> nfaCache.GetOrConvert "NFA nb-double-text illegal-chars" ps
            let ``illegal-patt`` = 
                this.``c-double-quote`` + GRP(this.``nb-double-text`` ps)
                |> nfaCache.GetOrConvert "NFA nb-double-text illegal-patt" ps
    ...
    member this.``c-single-quoted`` ps : ParseFuncResult<_> = 
        ...
        let processFunc ps =
            ...
            let patt = 
                ...
                | _  ->  failwith "The context 'block-out' and 'block-in' are not supported at this point"
                |>  nfaCache.GetOrConvert "nb-single-text" ps

            let ``illegal-patt`` = 
                this.``c-single-quote`` + GRP(this.``nb-single-text`` ps)
                |>  nfaCache.GetOrConvert "nb-single-text illegal-patt" ps
    ...
    member this.``c-flow-sequence`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let processFunc ps =
            let nfaseqstart = nfaCache.GetOrConvert "c-sequence-start" ps (this.``c-sequence-start`` + OPT(this.``s-separate`` ps)) 
            ps |> ParseState.``Match and Advance`` nfaseqstart (fun prs ->
                let prs = prs.SetStyleContext(this.``in-flow`` prs)

                let noResult prs =
                    prs |> ParseState.``Match and Advance`` HardValues.``NFA c-sequence-end`` (fun psx -> 
                ...
                |   FallibleOptionValue.Value  ->
                    ...
                    prs2 
                    |> ParseState.``Match and Advance`` HardValues.``NFA c-sequence-end`` (fun psx -> FallibleOption.Value (c, psx), pm)
    ...
    member this.``ns-s-flow-seq-entries`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let rec ``ns-s-flow-seq-entries`` (psp:ParseState) (lst:Node list) : ParseFuncResult<_> =
            ...
            let (nsflowseqentry,pm) =(this.``ns-flow-seq-entry`` psp)
            match  nsflowseqentry.Result with
            |   FallibleOptionValue.Value ->
                let (entry, prs) = nsflowseqentry.Data
                let lst = entry :: lst
                let prs = prs.SkipIfMatch (this.``NFA optional s-separate`` prs)
                let commaPattern = 
                    this.``c-collect-entry`` + OPT(this.``s-separate`` prs)
                    |>  nfaCache.GetOrConvert "c-collect-entry" prs
                match prs with 
    ...
    member this.``c-flow-mapping`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let processFunc ps =
            ps |> ParseState.``Match and Advance`` (this.``c-mapping-start`` + OPT(this.``s-separate`` ps) |> nfaCache.GetOrConvert "c-mapping-start" ps) (fun prs ->
                let prs = prs.SetStyleContext(this.``in-flow`` prs)
                let (mres,pm) = this.``ns-s-flow-map-entries`` prs

                let noResult prs = prs |> ParseState.``Match and Advance`` (HardValues.``NFA c-mapping-end``) (fun prs2 -> CreateMapNode (NonSpecific.NonSpecificTagQM) (getParseInfo ps prs2) [] |> this.ResolveTag prs2 NonSpecificQM (prs2.Location))

                match mres.Result with
                |   FallibleOptionValue.Value  -> 
                    let (c, prs2) = mres.Data
                    prs2 
                    |> ParseState.``Match and Advance`` (HardValues.``NFA c-mapping-end``) (fun prs2 -> FallibleOption.Value(c, prs2), prs2.Messages)
                    |> FallibleOption.ifnoresult(fun () ->
    ...
    member this.``ns-flow-map-entry`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let ``ns-flow-map-explicit-entry`` ps = 
            ps |> ParseState.``Match and Advance`` (this.``c-mapping-key`` + (this.``s-separate`` ps) |> nfaCache.GetOrConvert "c-mapping-key" ps) (this.``ns-flow-map-explicit-entry``)
        (ps |> ParseState.OneOf) {
    ...
    member this.``ns-flow-map-yaml-key-entry`` (ps:ParseState) =
        ...
        |   FallibleOptionValue.Value -> 
            ...
            let prs = prs.SkipIfMatch (this.``NFA optional s-separate`` prs) |> ParseState.ResetRestrictions
    ...
    member this.``c-ns-flow-map-empty-key-entry`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        |   FallibleOptionValue.Value -> 
            ...
            FallibleOption.Value((this.ResolvedNullNode prs, c), prs), pm   //  ``e-node``
        |   FallibleOptionValue.NoResult   -> FallibleOption.NoResult(), pm
        |   FallibleOptionValue.ErrorResult -> FallibleOption.ErrorResult(), pm
        | _ -> failwith "Illegal value for cnsflowmapseparatevalue"
        |> ParseState.TrackParseLocation ps
    ...
    member this.``c-ns-flow-map-separate-value`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        ps |> ParseState.``Match and Advance`` HardValues.``NFA c-mapping-value`` (fun prs ->
            if IsMatch(prs.Input.Data, (this.``ns-plain-safe`` prs) |> nfaCache.GetOrConvert "ns-plain-safe" prs) then FallibleOption.NoResult(), prs.Messages
            else
                let (nsflownode,pm) = prs |> ParseState.``Match and Advance`` (this.``s-separate`` prs |> nfaCache.GetOrConvert "s-separate" prs) (this.``ns-flow-node``)
    ...
    member this.``c-ns-flow-map-adjacent-value`` (ps:ParseState) : FallibleOption<_>*ParseMessage =
        ...
        ps |> ParseState.``Match and Advance`` (HardValues.``NFA c-mapping-value``) (fun prs ->
            let prs = prs.SkipIfMatch (this.``NFA optional s-separate`` ps)
...
//        let ps = ps.SetIndent ``n/a``
        ...
        |   FallibleOptionValue.Value -> 
            let (ck, prs) = nsflowyamlnode.Data
            let prs = prs.SkipIfMatch HardValues.``NFA optional s-separate-in-line``
...
//        let ps = ps.SetIndent ``n/a``
    ...
    member this.``ns-flow-yaml-content`` (ps:ParseState) : ParseFuncResult<_> = 
        ...
        let ``illegal-ns-plain`` ps = 
            ...
            |>  nfaCache.GetOrConvert "ns-plain-one-line illegal-ns-plain" ps
        let ``illegl multiline`` ps = 
            let memFunc ps =
                (this.``ns-plain-one-line`` ps) + OOM(this.``s-ns-plain-next-line`` ps)
            let callMemoized = this.Memoize memFunc
            callMemoized (1561, ps.n, ps.c) ps
            |>  nfaCache.GetOrConvert "ns-plain-one-line illegl multiline" ps
        let (preErr, pm) = 
            let pm = ps.Messages
        ...
        let ``ns-plain`` =
            this.``ns-plain`` ps
            |>  nfaCache.GetOrConvert "ns-plain" ps
        match preErr.Result with
        |   FallibleOptionValue.NoResult ->
            match (ps.c, ps) with
            |   Context.``Flow-out``, Regex4(``ns-plain``, postParseCondition) (mt, prs) 
            |   Context.``Flow-in``,  Regex4(``ns-plain``, postParseCondition) (mt, prs)  -> 
#if DEBUG
                ...
                |> this.ResolveTag prs NonSpecificQM (prs.Location)
                |> this.PostProcessAndValidateNode
            |   Context.``Block-key``, Regex3(``ns-plain``) (mt, prs) 
            |   Context.``Flow-key``,  Regex3(``ns-plain``) (mt, prs)  -> 
#if DEBUG
                logger (sprintf "> ns-plain value: %s" mt.FullMatch) prs
#endif
                let dl = ParseState.PositionDelta mt.FullMatch ||> DocumentLocation.Create
    ...
    member this.``ns-flow-yaml-node`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let processFunc ps =
            let ``ns-flow-yaml-content`` psp =
                psp 
                |> ParseState.``Match and Advance`` (this.``NFA s-separate`` psp) (this.``ns-flow-yaml-content``)
    ...
    member this.``c-flow-json-node`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let ``c-flow-json-content`` ps =
            ps |> ParseState.``Match and Advance`` (this.``NFA s-separate`` ps) (this.``c-flow-json-content``)
        ps.OneOf {
            either (this.``c-flow-json-content``)
    ...
    member this.``ns-flow-node`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let ``ns-flow-content`` ps =
            ps |> ParseState.``Match and Advance`` (this.``NFA s-separate`` ps) (this.``ns-flow-content``)
    ...
    member this.``c-b-block-header`` (ps:ParseState) = 
        ...
        let ``indent chomp`` ps : FallibleOption<int option * ParseState> * ParseMessage = 
            let p = HardValues.``NFA indent chomp``
            match ps with
            | Regex3(p)  (mt,prs) -> 
                let (i, c) = mt.ge2
        ...
        let ``chomp indent`` ps : FallibleOption<int option * ParseState>* ParseMessage = 
            let p = HardValues.``NFA chomp indent``
            match ps with
            | Regex3(p)  (mt, prs) -> 
                let (c, i) = mt.ge2
                FallibleOption.Value(indent  i, ps.SetChomping (chomp c) |> ParseState.Advance |> ParseState.TrackPosition mt.FullMatch), ps.Messages
        ...
        let ``illformed chomping`` ps : FallibleOption<int option * ParseState> * ParseMessage =
            let p = HardValues.``NFA illformed chomping``
            match ps with
            | Regex3(p)  (mt, prs) -> ps.AddErrorMessage <| MessageAtLine.CreateTerminate (ps.Location) MessageCode.ErrFoldedChompIndicator (lazy sprintf "Illegal chomp indicator '%s'" (mt.ge1))
            |   _ -> FallibleOption.NoResult(), ps.Messages
            ...
            let rec trimHead sin sout =
                ...
                |   h :: rest ->
                    if (h="") then
                        ...
                        |   _ -> trimMain sin sout
            trimHead slist []
        ps |> ParseState.``Match and Advance`` HardValues.``NFA pipe`` (fun prs ->
            let ``literal-content`` (ps:ParseState) =
                let ps = if ps.n < 1 then (ps.SetIndent 1) else ps
                let p = this.``l-literal-content`` ps
                match ps  with
                |   Regex3(p)  (m, prs) -> FallibleOption.Value(m.ge1, ps |> ParseState.TrackPosition m.FullMatch), ps.Messages
                |   _ -> FallibleOption.NoResult(), ps.Messages
    ...
    member this.``l-literal-content`` (ps:ParseState) = 
        ...
        |>  nfaCache.GetOrConvert "l-nb-literal-text" ps
    ...
    member this.``c-l+folded`` ps =
        ...
        ps |> ParseState.``Match and Advance`` (HardValues.``NFA c-folded``) (fun prs ->
    ...
    member this.``l-folded-content`` (ps:ParseState) =
        ...
        let callMemoized = this.Memoize memFunc
        callMemoized (182, ps.n, ps.c) ps
        |>  nfaCache.GetOrConvert "l-nb-diff-lines" ps
        ...
        let processFunc ps =
            ...
            if m < 1 then 
                ...
                let rec ``l+block-sequence`` (psp:ParseState) (acc: Node list) =
                    ...
                    let (clblockseqentry, pm) = pspf |> ParseState.``Match and Advance`` (this.``s-indent(n)`` pspf |> nfaCache.GetOrConvert "s-indent(n)" pspf) (this.``c-l-block-seq-entry``)
    ...
    member this.``c-l-block-seq-entry`` ps =
        ...
        ps |> ParseState.``Match and Advance`` HardValues.``NFA hyphen`` (fun prs ->
            if IsMatch(prs.Input.Data, (HardValues.``NFA ns-char``)) then // Not followed by an ns-char
    ...
    member this.``s-l+block-indented`` ps =
        ...
        let ``indented compact`` (ps:ParseState) =
            let psi = (ps.SetIndent (ps.m))
            ps |> ParseState.``Match and Advance`` (this.``s-indent(n)`` psi |> nfaCache.GetOrConvert "s-indent(n)" psi) (fun prs ->
        ...
        (ps |> ParseState.OneOf) {
            ...
            ifneitherfn (fun() ->
                let prs2 = ps.SkipIfMatch HardValues.``NFA e-node s-l-comments``
    ...
    member this.``ns-l-compact-sequence`` ps = 
        ...
        let rec ``ns-l-compact-sequence`` psp (acc: Node list) =
            ...
            if not(IsMatch(psp.Input.Data, (this.``s-indent(n)`` psp) |> nfaCache.GetOrConvert "s-indent(n)" psp)) then
                let ilen =
                    match HasMatches(psp.Input.Data, HardValues.``NFA ZOM s-space``) with
                    |   (true, mt) -> mt.Length
                    |   (false, _) -> 0
                psp.Input.Reset()
                ...
                //psp.Input.Reset()
                let (clblockseqentry, pm) = psp |> ParseState.``Match and Advance`` (this.``s-indent(n)`` psp |> nfaCache.GetOrConvert "s-indent(n)" psp) (this.``c-l-block-seq-entry``)
                match clblockseqentry.Result  with
                |   FallibleOptionValue.Value -> 
                    let (c, prs2) = clblockseqentry.Data
    ...
    member this.``c-l-block-map-explicit-entry`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        |   FallibleOptionValue.Value ->
            ...
            let noResult pm prs1  =
                prs1 |> ParseState.``Match and Advance`` (HardValues.``NFA e-node``) (fun prs2 -> FallibleOption.Value((ck, this.ResolvedNullNode prs2), prs2), pm)
            ...
            |   FallibleOptionValue.Value  -> 
                ...
                FallibleOption.Value((ck, cv), prs1), pm2
    ...
    member this.``c-l-block-map-explicit-key`` ps : ParseFuncResult<_> =
        ...
        ps |> ParseState.``Match and Advance`` (HardValues.``NFA c-mapping-key``) (fun prs ->
    ...
    member this.``l-block-map-explicit-value`` ps : ParseFuncResult<_> = 
        ...
        ps |> ParseState.``Match and Advance`` ((this.``s-indent(n)`` ps) + this.``c-mapping-value`` |> nfaCache.GetOrConvert "c-mapping-value" ps) (fun prs ->
            this.``s-l+block-indented`` (prs |> ParseState.SetStyleContext Context.``Block-out``)
        )
        |> ParseState.ResetEnv ps
        |> ParseState.TrackParseLocation ps
    ...
    member this.``ns-l-block-map-implicit-entry`` ps : ParseFuncResult<_> =
        ...
        let matchValue (ck, prs) =
            ...
            |   FallibleOptionValue.Value -> 
                ...
                FallibleOption.Value((ck, cv), prs2), pm
            |   FallibleOptionValue.NoResult   -> FallibleOption.NoResult(), pm
            |   FallibleOptionValue.ErrorResult -> FallibleOption.ErrorResult(), pm
        ...
        let noResult psp =
            if (ParseState.HasNoTerminatingError psp) then
                psp |> ParseState.``Match and Advance`` (HardValues.``NFA e-node``) (fun prs ->
        ...
        ps |> ParseState.``Match and Advance`` (HardValues.``NFA c-mapping-value``) (fun prs ->
            ...
            let noResult prs =
                if (ParseState.HasNoTerminatingError prs) then
                    prs |> ParseState.``Match and Advance`` (HardValues.``NFA e-node s-l-comments``) (fun prs ->
                        this.ResolveTag prs NonSpecificQM (prs.Location) (PlainEmptyNode (getParseInfo ps prs))
                    )
                else
    ...
    member this.``ns-l-compact-mapping`` ps =
        ...
        let rec ``ns-l-compact-mapping`` psp (acc: (Node * Node) list) =
            ...
            let (nslblockmapentry, pm) = psp |> ParseState.``Match and Advance`` (this.``s-indent(n)`` psp |> nfaCache.GetOrConvert "s-indent(n)" psp) (this.``ns-l-block-map-entry``)
            match nslblockmapentry.Result with
            |   FallibleOptionValue.Value  ->  
                let ((ck, cv), prs) = nslblockmapentry.Data
    ...
    member this.``s-l+flow-in-block`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        prs |> ParseState.``Match and Advance`` (this.``NFA s-separate`` prs) (fun prs ->
            let (nsflownode, pm) = (this.``ns-flow-node`` prs) 
            match nsflownode.Result with
            |   FallibleOptionValue.Value -> 
                let (c, prs2) = nsflownode.Data
                FallibleOption.Value(c, prs2.SkipIfMatch (HardValues.``NFA s-l-comments``)), pm 
            |   FallibleOptionValue.NoResult   -> FallibleOption.NoResult(), pm 
            |   FallibleOptionValue.ErrorResult -> FallibleOption.ErrorResult(), pm
            | _ -> failwith "Illegal value for nsflownode"
        )
    ...
    member this.``s-l+block-scalar`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let processFunc (ps:ParseState) =
            ...
            let ``literal or folded`` (psp:ParseState) =
                ...
                psp1 |> ParseState.``Match and Advance`` (this.``NFA s-separate`` psp1) (fun prs ->
    ...
    member this.``s-l+block-collection`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        let ``seq or map`` (pls:ParseState) =
            let pls = pls.SkipIfMatch(HardValues.``NFA s-l-comments``) 
        ...
        let ``optional spaced content with properties`` ps =
            ps |> ParseState.``Match and Advance`` (this.``NFA s-separate`` ps) (this.``content with properties`` ``seq or map``)
    ...
    member this.``l-bare-document`` (ps:ParseState) : ParseFuncResult<_> = 
        ...
        if ps.Errors = 0 then
            ...
            if IsMatch(ps.Input.Data, HardValues.``NFA c-forbidden``) then
                FallibleOption.NoResult(), ps.Messages
            else
                //ps.Input.Reset()
                ps 
    ...
    member this.``l-explicit-document`` (ps:ParseState) : ParseFuncResult<_> =
        ...
        if ps.Errors = 0 then
            ps 
            |> ParseState.``Match and Advance`` (HardValues.``NFA c-directives-end``) (fun prs ->
                (prs |> ParseState.OneOf)
                    {
                        either(this.``l-bare-document``)
                        ifneitherfn (fun () ->
                            let prs2 = prs.SkipIfMatch (HardValues.``NFA e-node s-l-comments``)
    ...
    member this.``l-yaml-stream`` (input:string) : Representation list= 
        ...
        let IsEndOfStream psp =
            ...
            if not(isEof()) then
                ...
                true
        ps |> 
        ParseState.``Match and Advance`` HardValues.``NFA l-document-prefix`` (fun psr ->
            ...
            let rec successorDoc (ps:ParseState, representations) =
                ...
                if not(IsEndOfStream ps) then
                    (ps |> ParseState.ResetDocumentParseState |> ParseState.OneOf) {
                        either (ParseState.``Match and Advance`` HardValues.``NFA l-document-suffix l-document-prefix`` (this.``l-any-document``))
                        either (ParseState.``Match and Advance`` HardValues.``NFA l-document-suffix l-document-prefix`` (fun psr -> if (IsEndOfStream psr) then (FallibleOption.Value(noResultNode, psr),psr.Messages) else (FallibleOption.Value(quitNode, psr), psr.Messages))) // for missing ``l-any-document``; which is optional
                        either (ParseState.``Match and Advance`` HardValues.``NFA l-document-prefix`` (this.``l-explicit-document``))
